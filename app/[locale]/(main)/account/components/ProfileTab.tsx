...
const ProfileTab = ({ customer, onRefresh }: { customer: any; onRefresh: () => void }) => {
    ...
    <div className="border border-black/10 rounded-2xl overflow-hidden bg-white">
      <div className="flex items-center justify-between px-6 py-5 bg-black/[0.02] border-b border-black/10">
        ...
        <button
          ...
          disabled={saving}
          className="flex items-center gap-2 px-4 py-2 rounded-full border border-black/20 text-sm hover:border-gold transition cursor-pointer disabled:opacity-50"
        >
          <FiEdit2 size={13} />
          {editing ? "Cancel" : "Edit"}
        </button>
      ...
      <div className="px-6 py-8 flex flex-col gap-6 bg-white">
        ...
        <div className="grid grid-cols-2 max-md:grid-cols-1 gap-4 bg-white">
          ...
          <div className="flex flex-col gap-1.5 bg-white">
            <label className="text-xs text-black/50">Email Address (Read Only)</label>
            <div className="relative">
              <FiMail size={13} className="absolute left-4 top-1/2 -translate-y-1/2 text-black/30" />
              <input
                disabled={true}
                value={form.email}
                className="w-full border border-black/15 rounded-xl pl-9 pr-4 py-3 text-sm outline-none disabled:bg-black/[0.02] disabled:text-black/60 bg-white"
              />
            </div>
          </div>
        ...
        {editing && (
          <button
            onClick={handleSave}
            disabled={saving}
            className="self-start px-8 py-3 rounded-full bg-navy text-white text-sm font-medium hover:opacity-90 transition cursor-pointer disabled:opacity-50"
          >
